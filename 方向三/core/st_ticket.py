...
class STService:
    ...
    def verify_st(self, ticket: dict, service: str,
                  expected_perm: Optional[dict] = None,
                  st_kind: Optional[str] = None,
                  claims_checker: Optional[Callable[[dict], bool]] = None,
                  now: Optional[float] = None,
                  replay_cache: Optional[Dict[str, float]] = None) -> dict:
        """验双 ST：签名 → 时效（30min 窗口）→ SName → st_kind → 单次使用
        → expected_perm 范围校验 → claims_checker。"""
        ...
        if expected_perm is not None:
            perm = ticket.get("perm", {})
            for k, v in expected_perm.items():
                if perm.get(k) != v:
        ...
        if self.audit is not None:
            self.audit.log("st_verify", "success", ticket["principal"],
                           ticket_id=tid)
